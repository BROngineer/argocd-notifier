...
| `internal/slack` | Slack rendering/client code (Block Kit, `chat.postMessage`/`chat.update`). Not wired into the core binary — a future standalone backend process wraps this behind `internal/remotebackend`'s HTTP contract (not built yet; see [remote-backends.md](remote-backends.md)'s Phase 5). |
...
- **No reference backend ships yet.** `internal/slack` has the rendering/client code, but nothing wraps it behind `internal/remotebackend`'s HTTP contract and runs it as a self-registering process yet — see [remote-backends.md](remote-backends.md)'s Phase 5. Until then, an event whose `backend` never registers simply fails that `(backend, recipient)` group every flush (logged, not fatal).
