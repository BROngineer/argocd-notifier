...
5. **Reference implementation: Slack as a standalone backend** — a new binary wrapping the existing `internal/slack` rendering/client code behind the `backendapi` server interface, self-registering and heartbeating. This is how Slack notifications work going forward — there is no in-process alternative left to fall back to.
