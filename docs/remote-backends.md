...
4. **Remote backend adapter + multi-backend routing**, split in two:
   ...
   - 4b. ✅ **`aggregator`/`main.go` wiring** — `aggregator.BackendResolver` composes the one compiled-in backend (checked by name first, matching `BACKEND=<name>`) with a fallback resolver (`remotebackend.Resolver`, backed by the registry) for any other name. `NewSessionPublisher` now takes a resolver instead of one fixed backend and no longer returns an error; the old construction-time thread-reply fail-fast moved to `aggregator.ValidateStaticBackend`, called from `main.go` against the compiled-in backend only, before anything else is wired. Session refs are keyed by `RefKey{Backend, Recipient}` (`groupAppsByBackendRecipient`), so two different backends sharing a recipient string never collide. `Upsert` prefers `notification.Notifier` when the resolved backend implements it, falling back to the existing `Backend`/`Updater` type-assertion path unchanged. An unresolvable backend name fails just that `(backend, recipient)` group for the current flush and logs an error — it does not abort the session or crash the process. Done (`feat/multi-backend-routing`).
